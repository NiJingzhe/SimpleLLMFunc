...
- **Runtime primitives**: expose controlled memory/fork APIs through `runtime.memory.*` and `runtime.fork.*`
...
> Agent guidance in tool description (sent to model): write direct snippets instead of standalone scripts, **do not** add `if __name__ == "__main__":`, `input()` is supported, and `reset_repl` does **not** delete attached memory state.
...
> Model-facing tool description is in English and explicitly states: reset only affects REPL variables and preserves registered runtime backends.
...
> Model-facing tool description is in English and clarifies that private names and runtime global (`runtime`) are excluded.
...
## Runtime primitives with SelfReference backend

Use explicit wiring for `SelfReference` as the host-side memory backend:
...
### Generic runtime backend / primitive registration

`PyRepl` also supports generic runtime extension points, not only `self_reference` pack wiring:
...
When `@llm_chat(...)` receives `self_reference`, the framework automatically appends a deduplicated memory contract to the end of the current system prompt.

For first-turn safety, if this memory key is empty, the current durable system prompt is seeded into `self_reference` before tool execution, so runtime memory reads are not empty and include a system message.

Inside `execute_code`, the agent accesses memory through runtime primitives:
...
```python
runtime.memory.set_system_prompt("agent_main", "You are a concise coding assistant.")
```
...
```python
print(runtime.memory.count("agent_main"))
print(runtime.memory.all("agent_main"))
print(runtime.memory.get("agent_main", 0))

runtime.memory.append("agent_main", {"role": "user", "content": "remember this"})
runtime.memory.insert("agent_main", 1, {"role": "assistant", "content": "inserted"})
runtime.memory.update("agent_main", 1, {"role": "assistant", "content": "updated"})
runtime.memory.delete("agent_main", 1)

runtime.memory.replace("agent_main", [
...
- `runtime.memory.count(key)`: return current message count for `key`.
- `runtime.memory.all(key)`: return a deep-copied snapshot of all messages.
- `runtime.memory.get(key, index)`: read one message by index.
- `runtime.memory.append(key, message)`: append one message to the tail.
...
- `runtime.memory.delete(key, index)`: remove one message by index.
- `runtime.memory.replace(key, messages)`: replace full history with a validated list.
- `runtime.memory.clear(key)`: clear all messages for `key`.
- `runtime.memory.get_system_prompt(key)`: read the latest system prompt text.
- `runtime.memory.set_system_prompt(key, text)`: overwrite current system prompt.
- `runtime.memory.append_system_prompt(key, text)`: append text to system prompt.

Forgetting note:

- Forgetting memory is **not** `reset_repl`.
- `reset_repl` only clears Python variables in the REPL namespace.
- Forget memory by deleting message records with `runtime.memory.delete(...)`, `runtime.memory.replace(...)`, or `runtime.memory.clear(...)`.
...
- TUI runtime memory demo: `examples/tui_runtime_memory_example.py`
- TUI runtime fork demo: `examples/tui_runtime_fork_subagent_example.py`
