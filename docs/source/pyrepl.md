...
- **Runtime primitives**: expose controlled self-reference APIs through `runtime.selfref.history.*` and `runtime.selfref.fork.*`
- **Origin-aware event stream**: each `EventYield` carries `origin` metadata for stable main/fork routing in TUI or custom UIs
...
`PyRepl` also supports generic runtime extension points, not only `selfref` pack wiring:
...
```python
runtime.selfref.history.set_system_prompt("You are a concise coding assistant.")
```
...
print(runtime.selfref.history.count())
print(runtime.selfref.history.all())
print(runtime.selfref.history.get(0))

runtime.selfref.history.append({"role": "user", "content": "remember this"})
runtime.selfref.history.insert(1, {"role": "assistant", "content": "inserted"})
runtime.selfref.history.update(1, {"role": "assistant", "content": "updated"})
...
- `runtime.selfref.history.active_key()`: resolve active key for current execution context.
- `runtime.selfref.history.count(key=None)`: return current message count.
- `runtime.selfref.history.all(key=None)`: return a deep-copied snapshot of all messages.
- `runtime.selfref.history.get(index, key=None)`: read one message by index.
- `runtime.selfref.history.append(message, key=None)`: append one message to the tail.
