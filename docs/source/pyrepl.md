...
- **长输出自动截断**：`execute_code` 已启用 `too_long_to_file`，当输出超过 4000 tokens 时自动保存到临时文件并截断返回
- **Runtime 原语**：通过 `runtime.selfref.history.*` 与 `runtime.selfref.fork.*` 暴露受控的 self-reference API
...
> **长输出自动截断**：`execute_code` 已启用 `too_long_to_file` 功能。当代码输出超过 4000 tokens 时，完整结果会自动保存到临时文件（路径会在 `<system-reminder>` 中告知），返回内容会被截断到前 4000 tokens。这避免了超长输出导致的上下文溢出问题。
...
在 `execute_code` 中通过 runtime 原语访问记忆：
...
### 将持久记忆追加到 system prompt

这是最常见且推荐的模式：把用户偏好落到 system prompt 记忆中。
...
- `runtime.selfref.history.active_key()`: 获取当前执行上下文的 active key。
- `runtime.selfref.history.count(key=None)`: 返回当前消息数量。
- `runtime.selfref.history.all(key=None)`: 返回全部消息的深拷贝快照。
- `runtime.selfref.history.get(index, key=None)`: 读取指定索引消息。
- `runtime.selfref.history.append(message, key=None)`: 追加消息到末尾。
- `runtime.selfref.history.insert(index, message, key=None)`: 在指定位置插入消息。
- `runtime.selfref.history.update(index, message, key=None)`: 替换指定索引消息。
- `runtime.selfref.history.delete(index, key=None)`: 删除指定索引消息。
- `runtime.selfref.history.replace(messages, key=None)`: 用新列表替换全部历史。
- `runtime.selfref.history.clear(key=None)`: 清空非 system 消息并保留当前 system prompt。
