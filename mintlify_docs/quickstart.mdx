...
<Step title="安装 Agent Skill 包（推荐）">

如果你会配合编码 Agent 一起使用 SimpleLLMFunc，建议在安装完成后立刻导出内置 skill 包。这个 skill 会告诉 Agent 如何正确使用本框架、如何查文档、以及如何组织常见工作流。
...
如果你的 Agent 需要参与框架开发、修改底层实现或补充测试，也可以导出开发型 skill：
...
<Callout type="info">
支持的 skill 类型有 `usage`、`developer` 和 `dev`。如果目标目录已存在，可加 `--force` 覆盖。
</Callout>
...
SimpleLLMFunc 中的所有装饰器（如 `@llm_function`、`@llm_chat`、`@tool`）都要求装饰 `async def` 定义的函数；使用时请在异步上下文中通过 `await` 调用，或使用 `asyncio.run` 启动顶层任务。
</Callout>
...
<AccordionGroup>
  <Accordion title="API 密钥配置错误怎么办？">
    请检查 `provider.json` 文件中的 API 密钥是否正确，确保密钥有效且有足够的配额。
  </Accordion>
  <Accordion title="模型返回格式不正确怎么办？">
    对于复杂返回类型，框架会要求模型输出 XML 结构。若模型经常输出不合规 XML，建议切换更强模型，或在 DocString 中强化格式约束。
  </Accordion>
  <Accordion title="如何添加更多工具？">
    使用 `@tool` 装饰器定义工具函数，然后在 `@llm_function` 的 `toolkit` 参数中传入工具列表。
  </Accordion>
  <Accordion title="如何实现异步调用？">
    SimpleLLMFunc 中的装饰器只支持 `async def` 定义的函数，因此需要在异步上下文中通过 `await` 调用，或在脚本入口使用 `asyncio.run(...)`。
  </Accordion>
  <Accordion title="什么是动态模板参数？">
    动态模板参数允许你在调用时通过 `_template_params` 动态设置 DocString 中的占位符，让同一个函数适应不同场景。
  </Accordion>
  <Accordion title="如何使用动态模板参数？">
    ```python
...
<CardGroup>
  <Card title="查看示例代码" href="/examples">
    从 `examples/` 目录继续扩展到更多场景。
  </Card>
  <Card title="阅读使用指南" href="/guide">
    按任务和模块继续深入高级功能。
  </Card>
  <Card title="开始构建自己的应用" href="/detailed_guide/llm_function">
    从 `llm_function` 或 `llm_chat` 开始构建自己的流程。
  </Card>
  <Card title="参与贡献" href="/contributing">
    反馈问题、提交示例或补充文档。
  </Card>
</CardGroup>
