...
`llm_function` 是 SimpleLLMFunc 的核心入口之一。它把一个异步 Python 函数包装成“由 LLM 执行的函数”，你只需要定义函数签名、返回类型和 docstring，框架就会负责构造 prompt、调用模型、解析结果。

<CardGroup>
  <Card title="智能参数传递">自动把函数参数转换成 LLM 能理解的输入。</Card>
  <Card title="类型安全">按返回类型自动解析成 `str`、`dict`、Pydantic 模型等。</Card>
  <Card title="工具集成">可为函数挂载工具，让模型调用外部能力。</Card>
  <Card title="模板扩展">支持自定义 prompt 模板与动态模板参数。</Card>
  <Card title="事件流">支持 `enable_event=True` 观察调用过程和 Token 使用量。</Card>
</CardGroup>

## 重要说明

<Callout type="warning">
`llm_function` 只能装饰 `async def` 定义的异步函数，返回的也是可 `await` 的协程；请在异步上下文中调用，或在脚本入口使用 `asyncio.run()`。
</Callout>
...
<AccordionGroup>
  <Accordion title="基础参数">
    ...
    - `max_tool_calls`：可选，最大工具调用次数。默认 `None`，表示框架不主动限制
  </Accordion>
  <Accordion title="Prompt 模板参数">
    - `system_prompt_template`：自定义系统提示模板
    - `user_prompt_template`：自定义用户提示模板
  </Accordion>
  <Accordion title="事件流与 LLM 参数">
    - `enable_event=False`：直接返回解析后的结果
    - `enable_event=True`：返回异步生成器，yield `ReactOutput`
    - 其他 `llm_kwargs` 会直接传给底层模型接口，例如 `temperature`、`top_p`
    - 可传入 `retry_times` 控制空响应重试次数，默认 2
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="系统提示模板占位符">
    - `{function_description}`：函数文档字符串内容
    - `{parameters_description}`：函数参数及其类型描述
    - `{return_type_description}`：返回值类型描述
  </Accordion>
  <Accordion title="用户提示模板占位符">
...
<CardGroup>
  <Card title="动态角色切换">同一个函数可以扮演不同角色。</Card>
  <Card title="灵活任务适配">根据调用上下文调整任务类型。</Card>
  <Card title="透明处理">`_template_params` 不会传给模型，只参与模板渲染。</Card>
  <Card title="错误兜底">模板参数不完整时，会回退到原始 docstring。</Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="构造提示词">
    docstring 作为系统提示核心，参数会被格式化成用户输入；如果提供了模板，会在这一步应用。
  </Step>
  <Step title="调用 LLM 和工具">
    框架把 prompt 发给模型；如果模型触发工具调用，框架会自动执行工具并继续 ReAct 流程。
  </Step>
  <Step title="解析返回值">
    把模型输出解析为指定的返回类型，再返回给调用方。
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="类型转换支持">
    - 基本类型：`str`、`int`、`float`、`bool`
    - 容器类型：`List`、`Dict`
    - Pydantic 模型
    - 其他类型默认回退到文本结果
  </Accordion>
  <Accordion title="输出格式约束">
    - 简单返回类型通常使用纯文本约束
    - 复杂返回类型（Pydantic、`List`、`Dict`、`Union`）会使用更严格的结构化约束
  </Accordion>
  <Accordion title="错误处理机制">
    - 空响应自动重试
    - 调用异常捕获和日志记录
    - 类型转换失败时提供兜底处理
  </Accordion>
...
<Callout type="info">
下面保留原始代码示例，便于直接复制运行。你可以先看标题，再跳到自己最接近的场景。
</Callout>
...
<Callout type="warning">
`llm_function` 本身不提供流式文本输出。即使启用事件流，`ResponseYield` 也只会在最终解析完成后返回一次结果。
</Callout>
...
**详细文档**：请参考 [事件流系统文档](/detailed_guide/event_stream) 了解完整的事件类型、使用示例和最佳实践。
