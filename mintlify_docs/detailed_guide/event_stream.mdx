...
<Tabs>
  <Tab title="llm_chat 默认模式">
    ```python
    async for chunk, updated_history in chat("Hello"):
        print(chunk)
    ```
  </Tab>
  <Tab title="llm_chat 事件流模式">
    ```python
    async for output in chat("Hello"):
        if isinstance(output, ResponseYield):
            print(output.response)
  ...
  <Tab title="llm_function 默认模式">
    ```python
    result = await analyze_text("Hello")
    print(result)
    ```
  </Tab>
  <Tab title="llm_function 事件流模式">
    ```python
...
更完整的说明请参阅 [中断与取消](/detailed_guide/abort)。
