...
<CardGroup>
  <Card title="类型推断">自动从函数签名中提取参数类型和说明。</Card>
  <Card title="Docstring 解析">支持从 docstring 中抽取参数描述。</Card>
  <Card title="JSON Schema 生成">自动生成符合 Function Calling 的工具描述。</Card>
  <Card title="多返回类型">支持基本类型、结构化数据和多模态返回。</Card>
  <Card title="两种创建方式">支持装饰器方式和继承方式。</Card>
  <Card title="长文本截断">超长文本结果可自动落盘并截断回传。</Card>
</CardGroup>

### 支持的数据类型（参数类型）

<AccordionGroup>
  <Accordion title="支持的参数类型">
  ...
  <Accordion title="参数类型限制">
    LLM 提供给工具的参数本质上仍然是 JSON 支持的数据类型，因此 `Tuple`、`Set` 这类容器通常不适合作为工具参数。
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="基本返回类型">
    - `str`
    - JSON 可序列化对象：`dict`、`list`、`int`、`float`、`bool`、`None`
    - Pydantic 模型
  </Accordion>
  <Accordion title="多模态返回类型">
    - `ImgUrl`
    - `ImgPath`
    - `Tuple[str, ImgUrl]`
    - `Tuple[str, ImgPath]`
...
<Steps>
  <Step title="框架估算 token 数量">
    当工具返回 `str` 时，框架会估算其 token 大小。
  </Step>
  <Step title="超过阈值时自动落盘">
...
- 仅在工具可能返回大量文本时启用
- 启用后，agent 需要具备文件读取能力，例如 `FileToolset` 或 `PyRepl`
- 对于结构化返回值（`dict`、`list`）该功能不会生效
