...
<AccordionGroup>
  <Accordion title="LLM_Interface 抽象基类">
    `LLM_Interface` 是所有 LLM 实现的抽象基类，定义统一的接口规范。它的目标是把调用方式和返回形式收敛到一致的接口上。`OpenAICompatible` 与 `OpenAIResponsesCompatible` 都继承自它。
  ...
  <Accordion title="OpenAIResponsesCompatible 适配器">
    `OpenAIResponsesCompatible` 用于对接 OpenAI Responses API，同时保持 `@llm_function` / `@llm_chat` 的装饰器使用方式不变。
...
<Tabs>
  <Tab title="OpenAICompatible：从配置文件加载">
    ```python
  ...
  <Tab title="OpenAICompatible：手动创建实例">
    ```python
  ...
  <Tab title="OpenAIResponsesCompatible：从配置文件加载">
    ```python
  ...
  <Tab title="OpenAIResponsesCompatible：手动创建实例">
    ```python
