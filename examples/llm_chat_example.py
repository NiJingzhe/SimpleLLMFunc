...
async def async_simple_chat(history: Optional[List[Dict[str, str]]] = None, message: str = ""):
    """
...
async def async_programming_assistant(history: Optional[List[Dict[str, str]]] = None, code: str = "", question: str = ""):
    """
    异步编程助手：我专门帮助解决编程问题。
    我可以：
    - 解释代码逻辑
...
async def async_chat_example():
    ...
    full_response = ""
    async for content, updated_history in async_simple_chat(history=history, message="编写一个Python函数来计算斐波那契数列"):
        if content:
            console.print(content, end="")
            full_response += content
        else:
