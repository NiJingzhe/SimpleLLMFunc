...
- `provider.json` 采用 **提供商 -> 模型配置列表** 的结构
- `model_name` 会作为 `models[provider][model_name]` 的索引键
...
SimpleLLMFunc 提供了内建日志系统。运行 Demo 后，您可以：

1. 查看控制台输出日志（含 trace_id）
2. 检查 `LOG_DIR/application.log` 的结构化日志（默认 `logs/application.log`）
3. 若启用 PyRepl，查看 `LOG_DIR/pyrepl/<instance_id>/executions.jsonl` 的执行审计

每个函数调用都会生成唯一的 `trace_id`，便于追踪和调试。
...
A: 对于复杂返回类型，框架会要求模型输出 XML 结构。若模型经常输出不合规 XML，建议切换更强模型或在 DocString 中强化格式约束。
